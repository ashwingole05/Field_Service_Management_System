...
public class ServiceRequestController {
    ...
    // MANAGER / DISPATCHER - VIEW ALL REQUESTS
    ...
    // MANAGER / DISPATCHER - VIEW REQUEST BY ID
    @PreAuthorize("hasAuthority('REVIEW_REQUEST')")
    @GetMapping("/{id}")
    ...
    // MANAGER / DISPATCHER - FILTER BY STATUS
    @PreAuthorize("hasAuthority('REVIEW_REQUEST')")
    ...
    getByStatus(
        ...
        return ResponseEntity.ok(
                serviceRequestService.getByStatus(status)
        );
    }

    // MANAGER / DISPATCHER - MARK REQUEST IN REVIEW
    @PreAuthorize("hasAuthority('REVIEW_REQUEST')")
    @PutMapping("/{id}/review")
    public ResponseEntity<ServiceRequest> reviewRequest(
            @PathVariable Long id) {

        return ResponseEntity.ok(
                serviceRequestService.markInReview(id)
        );
    }

    // MANAGER / DISPATCHER - CONVERT REQUEST TO WORK ORDER
    @PreAuthorize("hasAuthority('CONVERT_REQUEST')")
    @PostMapping("/{id}/convert")
    public ResponseEntity<ServiceRequest> convertToWorkOrder(
            @PathVariable Long id) {
    ...
    // MANAGER / DISPATCHER - CLOSE REQUEST
    @PreAuthorize("hasAuthority('CLOSE_REQUEST')")
    @PutMapping("/{id}/close")
    public ResponseEntity<ServiceRequest> closeRequest(
            @PathVariable Long id) {

        return ResponseEntity.ok(
                serviceRequestService.closeRequest(id)
        );
    }

    // MANAGER / DISPATCHER - CANCEL REQUEST
    @PreAuthorize("hasAuthority('CANCEL_REQUEST')")
